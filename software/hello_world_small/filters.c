...
static inline int32_t ma(int32_t *buf, int32_t *sum, int *idx,
                         int n, int sh, int32_t x) {
    *sum += x - buf[*idx];
    buf[*idx] = x;
...
static int32_t bph_b[BPH_N];
static int32_t bph_s;
static int bph_i;

static int32_t bpl_b[BPL_N];
static int32_t bpl_s;
static int bpl_i;
...
void apply_filter(int32_t *left, int32_t *right, uint32_t mode) {
    switch (mode) {
        case FILTER_LOWPASS:
            filter_lowpass(left, right);
            break;

        case FILTER_HIGHPASS:
            filter_highpass(left, right);
            break;

        case FILTER_BANDPASS:
            filter_bandpass(left, right);
            break;
